...
app.get("/tickets/:id", (req, res) => {
    ...
    db.query(sql, [ticketId], (error, results) => {
        if(error){
            ...
            return res.status(500).json({error: "Failed to get open ticket"});
        }
        if(results.length === 0){
            return res.status(404).json({error: "ticket not found"});
        }
...
app.post("/tickets", async (req, res) => {
    ...
    db.query(sql, [title, description, ticketPriority, ticketStatus, submitted_by, assignedTo, deptId], async (error, results) => {
        ...
        // automaticly log this action to mongodb
        try{
...
// POST / login - validated creditials and returns user info with role
app.post("/login", async (req, res) => {
    ...
    db.query(sql, [email], async (error, results) => {
        ...
        // automaticlly log the loginaction to MongoDB
        try{
            const mongoDb = getMongo();
            await mongoDb.collection("activity_logs").insertOne({
                action: "user_login",
...
// POST / ticket-notes - adding a note to a ticket in mongodb
...
// POST /activity-logs - manuelly creates an activity log in MongoDb
